...
def plot_nu(x,y,s,folderName,t,perf_plate=False,perf_pts=False,internal_geometry=False,boundary=False):
    nu = 1 - mean(isnan(s),axis=2).T
    if internal_geometry: nu = ma.masked_where(boundary.T,nu)
    plt.clf()
...
def plot_u(x,y,s,u,v,folderName,t,nm,IC_mode,perf_plate,perf_pts,boundary):
    ...
    if IC_mode == 'slope': u = amax(u) - u # lagrangian
...
def plot_T(x,y,s,T,folderName,t,boundary_temperature,inlet_temperature):
    ...
    mask = sum(isnan(s),axis=2) > 0.95*nm
    plt.clf()
    plt.pcolormesh(x,y,ma.masked_where(mask,nanmean(T,axis=2)).T,cmap=bwr,vmin=boundary_temperature,vmax=inlet_temperature)
    plt.axis('off')
    plt.xlim(x[0],x[-1])
    plt.ylim(y[0],y[-1])
    plt.subplots_adjust(left=0,right=1,bottom=0,top=1)
